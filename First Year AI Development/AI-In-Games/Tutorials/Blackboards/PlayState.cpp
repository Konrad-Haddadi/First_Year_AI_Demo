...
void PlayState::Load()
{
	...
	player = new SpartanAgent("Player", table ,map, new GotoPointBehaviour, 150, DARKGREEN, m_stateManager, m_objectManager);
	...
	for (int i = 0; i < max; i++)
	{
		...
		agent.push_back(new CovenantBaseAgent(table, map, GetRandomValue(CovenantType::Grunt, CovenantType::EliteC)
	...
	/*Node* node = map->GetMap()[0];
	node->type = NodeType::Empty;

	CovenantBaseAgent* DearLeader = new CovenantBaseAgent(table, "ATTILA DA Fanta", map, CovenantType::EliteC, 50, m_stateManager, m_objectManager);

	agent.push_back(DearLeader);
	DearLeader->SetCurrentNode(node->pos);
	DearLeader->SetPos(node->pos);
