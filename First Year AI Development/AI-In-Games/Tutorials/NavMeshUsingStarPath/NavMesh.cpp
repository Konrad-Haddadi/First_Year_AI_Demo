...
void NavMesh::Draw()
{
	...
	for (auto o : GetObstacles())
		DrawRectangle(o.rect.x, o.rect.y, o.rect.width, o.rect.height, obstacleColor);
}
...
{	
	...
	{
		vec2* point = portals[i].DoesIntersect({currentPoint,  _end});

		if (point)
			currentPoint = *point;		
		else
			currentPoint = portals[i].GetClosestPoint((currentPoint + _end) / 2.0f);
		
		
		smoothPath->push_back(currentPoint);
	}

	_agent->SetPortalPath(portals);
	smoothPath->push_back(_end);
}

Rectangle* NavMesh::DoesCollide(vec2 _start, vec2 _end)
{
	bool top = false;
	bool right = false;
	bool left = false;
	...
	for (auto o : m_obstacles)
	{
		...
		top = CheckCollisionLines({ _start.x, _start.y }, { _end.x, _end.y },
			{ o.rect.x, o.rect.y }, { o.rect.x + o.rect.width, o.rect.y }, &vec);

		right = CheckCollisionLines({ _start.x, _start.y }, { _end.x, _end.y },
			{ o.rect.x + o.rect.width, o.rect.y }, { o.rect.x + o.rect.width, o.rect.y + o.rect.height }, &vec);

		left = CheckCollisionLines({ _start.x, _start.y }, { _end.x, _end.y },
			{ o.rect.x, o.rect.y }, { o.rect.x, o.rect.y + o.rect.height }, &vec);

		bot = CheckCollisionLines({ _start.x, _start.y }, { _end.x, _end.y },
			{ o.rect.x, o.rect.y + o.rect.height }, { o.rect.x + o.rect.width, o.rect.y + o.rect.height }, &vec);
