...
HashTable::~HashTable()
{
	...
	{
		for (int j = 0; j < m_letters[i].size(); j++)
		{
			m_letters[i].clear();
...
void HashTable::Load()
{
	for (float y = 0; y < 5; y++)
	{
		for (float x = 0; x < 8; x++)
		{
			m_letters.push_back(vector<char>());
	...
	for (int i = 0; i < m_letters.size(); i++)
	{
		for (int j = 0; j < GetRandomValue(20, 100); j++)
		{
			int amountCap = GetRandomValue(65, 90);
			int amountLower = GetRandomValue(97, 122);

			int lowOrCap = GetRandomValue(0, 1);

			if (lowOrCap == 0)
				m_letters[i].push_back(amountCap);
			else
				m_letters[i].push_back(amountLower);
		}
	}
}

void HashTable::RefreshQuad(int _quad)
{	
	m_letters[_quad].clear();
	...
	for (int j = 0; j < GetRandomValue(20, 100); j++)
	{
		...
		if (lowOrCap == 0)
			m_letters[_quad].push_back(amountCap);
		else
			m_letters[_quad].push_back(amountLower);
	}	
}
...
vector<char>* HashTable::GetLetters(int _quad)
{	
	return &m_letters[_quad];
}
